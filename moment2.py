...
function flask_moment_render(elem) {
    $(elem).text(eval('moment2("' + $(elem).data('timestamp') + '").' + $(elem).data('format') + ';'));
...
$(document).ready(function() {
    ...
    @staticmethod
    def lang(language):
        return _moment2.locale(language)
    ...
    def fromTime(self, timestamp, no_suffix=False, refresh=False):
        return self._render("from(moment2('%s'),%s)" %
...
class Moment2(object):
    ...
    def init_app(self, app):
        if not hasattr(app, 'extensions'):
            app.extensions = {}
        app.extensions['moment2'] = _moment2
        app.context_processor(self.context_processor)

    @staticmethod
    def context_processor():
        return {
            'moment2': current_app.extensions['moment2']
        }

    def create(self, timestamp=None):
        return current_app.extensions['moment2'](timestamp)
